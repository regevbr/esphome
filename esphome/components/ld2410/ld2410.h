...
static const std::map<std::string, uint8_t> baud_rate_enum_to_int_{{"9600", 1},
                                                               {"19200", 2},
                                                               {"38400", 3},
                                                               {"57600", 4},
                                                               {"115200", 5},
...
class LD2410Component : public Component, public uart::UARTDevice {
  ...
  SUB_LAMBDA_SELECT(baud_rate, {
    ...
    this->set_timeout(200, [this, state]() {
      this->restart_();
      this->set_timeout(1000, [state]() {
        ESP_LOGE(TAG, "set_baud_rate_(): change baud rate component config to %s and reinstall", state.c_str());
      });
    });
  })
#endif
